...
function Card({ title, meta, label, selected = false, className = "", children, ...rest }: CardProps) {
  const classes = ["ss-card", selected ? "is-selected" : "", className].filter(Boolean).join(" ");
  return (
